...
const TaskList = props => {
    ...
    const TimeCircle = props => {
        return Array(props.tomatos)
            .fill(0)
            .map((_, k) => <Circle key={k} />);
    };
    ...
    return (
        <>
            ...
            <Content>
                <Nav>
                    <NavBtn onClick={() => setNav('todo')} active={nav === 'todo'}>
                        TODO
                    </NavBtn>
                    <NavBtn onClick={() => setNav('done')} active={nav === 'done'}>
                        DONE
                    </NavBtn>
                </Nav>
                ...
                <ListContainer>
                    {props.task.map((item, key) => (
                        <List key={key}>
                            ...
                            <ExpandContainer>
                                <FontAwesomeIcon icon={faArrowDown} size="lg" color="#ACACAC" />
                            </ExpandContainer>
                        </List>
                    ))}
                </ListContainer>
